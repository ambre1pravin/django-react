...
handleQuotationTemplateSave(id,name){

    var products = [];

    var product_rows = $('tbody.order').find('.product_row');

    product_rows.each(function(){
      var product_raw = []
      $(this).find('input').each(function(){
        product_raw.push({'name':$(this).attr('name'),'value':$(this).val()})
      });
      products.push({product_raw});
    });

    var optional = [];

    var optional_rows = $('tbody.optional').find('.product_row');

    optional_rows.each(function(){
      var product_raw = []
      $(this).find('input').each(function(){
        product_raw.push({'name':$(this).attr('name'),'value':$(this).val()})
      });
      optional.push({product_raw});
    });

    let total_tax_amt     = this.state.total_tax_amt;
    let untaxed_amt = this.state.untaxed_amt;

    let optotal_tax_amt =this.state.optotal_tax_amt;
    let opuntaxed_amt = this.state.opuntaxed_amt;
    var Data = {'id':parseInt(id) ,'name':name,'tax_amt':total_tax_amt,'untaxed_amt':untaxed_amt,'optax_amt':optotal_tax_amt,'opuntaxed_amt':opuntaxed_amt,'optional_products': optional, 'products':products,'optional_products': optional}
    $.ajax({
        type: "POST",
        cache: false,
        ...
        success: function (data) {
          if(data.success === true){
            ...
            toast.success("Quotation Template Successfully Save!", {position: toast.POSITION.TOP_RIGHT, toastId: "handleQuotationTemplateSave"});
          }
        }
      });
}
  ...
  handleSubmit(quot_status=''){
    ...
    handleselectedTaxes(items_type, index, id, name, tax_value, tax_computation) {
        let items_array;
        if (items_type == 'order') {
            items_array = this.state.items;
        } else if (items_type == 'optional') {
    ...
    updateQty(items_type, index, value, tax_value) {
        ...
        if (items_type == 'order') {
            ...
            for (i = 0; i < items.length; i++) {
                opuntaxed_amt = opuntaxed_amt + items[i].subtotal;
                optotal_tax_amt = optotal_tax_amt + items[i].tax_amt;
            }
  ...
  setCreatedTemplate(id, value){
    this.setState({
            selected_tmpl_id :id ,
            selected_tmpl_name : value
            })
  }
    ...
    handleTemplateSelection(id, name) {
        ...
        if(id > 0){
            ...
            $.ajax({
                type: "POST",
                cache: false,
                url: '/quotation/getTemplateDataByID/',
                data: {
                    csrfmiddlewaretoken: getCookie('csrftoken'),
                    ajax: true,
                    id: id
                ...
                success: function (data) {
                    if (data.success === true) {
                        this.setState({
                            notes: data.terms_and_codition !== undefined && data.terms_and_codition != null ? data.terms_and_codition : '',
                            expiration_date: data.expiration_date !== undefined && data.expiration_date != null ? data.expiration_date : '',
                        })
                        if (data.products !== undefined && data.products.length > 0) {
                            ...
                            data.products.forEach(function (element, i) {
                                var unit_price = element.product_up
                                var discount = element.discount
                                var qty = element.product_qty
                                var discount_amt = 0;
                                var subtotal = qty * unit_price
                                ...
                                untaxed_amt += subtotal;
                                var tax_amt = 0;
                                var tax_id = element.product_tax_id;
                                if (tax_id > 0) {
                                    var json_taxes_data = taxes_list;
                                    var current_item = {'pro_qty': element.product_qty, 'subtotal': subtotal}
                                    tax_amt = this.handleTaxCalculation(element.product_tax_value, element.product_tax_computation, current_item);
                                }
                                ...
                                temp_arr.push({
                                    ...
                                    'selected_tax_name': element.product_tax_id != null ? element.product_tax_name : '',
                                    'selected_tax_id': element.product_tax_id != null ? element.product_tax_id : '',
                                    'selected_tax_value': element.product_tax_id != null ? element.product_tax_value : '',
                                    'selected_tax_name_computation': element.product_tax_id != null ? element.product_tax_computation : '',
                                    'json_taxes': taxes_list,
                                    'tax_amt': tax_amt,
                                    'subtotal': subtotal,
                                    'json_product': products_list,
                                    'json_uom': element.json_uom
                                });
                            }.bind(this));
                        ...
                        if (data.optionals !== undefined && data.optionals.length > 0) {
                            var temp_arr = []
                            var opuntaxed_amt = 0;
                            var optotal_tax_amt = 0;
                            data.optionals.forEach(function (element, i) {
                                var unit_price = element.product_up
                                var discount = element.discount
                                var qty = element.product_qty
                                var discount_amt = 0;
                                ...
                                subtotal = subtotal - discount_amt;
                                opuntaxed_amt += subtotal;
                                var optax_amt = 0;
                                var tax_id = element.product_tax_id;
                                if (tax_id > 0) {
                                    var json_taxes_data = taxes_list;
                                    var current_item = {'pro_qty': element.product_qty, 'subtotal': subtotal}
                                    optax_amt = this.handleTaxCalculation(element.product_tax_value, element.product_tax_computation, current_item);
                                }
                                optotal_tax_amt += optax_amt;
    ...
    set_notification(status) {
        if (status === true) {
            toast.success("Product Saved!", {position: toast.POSITION.TOP_RIGHT, toastId: "handleQuotationTemplateSave"});
        }
    }

    setCreatedProductUOM(index, id, value) {
        var items_type = this.state.temp_item_type
        if (items_type == 'order') {
            var items = this.state.items;
            var items_array = 'items'
        }else if (items_type == 'optional') {
        ...
        items[index].selected_uom = value;
        items[index].selected_uom_id = id;
        this.setState({
            items_array: items
        })
    ...
    /* end: unit of measure modal handling*/
            ...
            <table>
               <tbody>
                ...
                <tr>
                    ...
                    <td>
                        { calculation_for === 'order' ?
                                <span>{result ? this.state.total_tax_amt.toFixed(2) : 0.00}</span>
                            : calculation_for === 'optional' ?
