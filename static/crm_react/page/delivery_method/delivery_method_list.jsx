...
class DeliveryMethodList extends React.Component {
...
handleView(view_id){
     browserHistory.push(BASE_FULL_URL+"/delivery/method/view/"+view_id);
  }
      ...
      var Data = [{'name':'limit', 'value':limit}, {'name':'offset', 'value':offset}];
       ...
       $.ajax({
            ...
            success: function (data) {
                  ...
                  if(data.delivery.length==0 && page>1){
                    this.setState({current_page: page-1}, ()=>{this.getDeliveryData()})
                  }
                  else if(data.delivery.length>0){
                    var total_page =  Math.floor(data.total_count/limit);
                      if(data.total_count%limit!=0){
                            total_page +=1;                     
                      }
                      var page_start = offset+1;
                      var page_end   = offset+ data.delivery.length;
    ...
    var can_remomve = true;
      ...
      if(quot_ids.length>0){
        $.ajax({
            type: "POST",
            cache: false,
            url: '/delivery/method/deletedelivery/',
            data: {
              ajax: true,
            ...
            success: function (data) {
              if(data.success == true){
                this.setState({current_page:current_page},()=>{this.getDeliveryData()})
              }
                      ...
                      <div className="row top-actions">
                          <div className="col-xs-12 col-sm-12">
                              <ul className="breadcrumbs-top">
                                  <li> <Link to={'/quotations_dashboard/'}  className="breadcumscolor" title={translate('quotations_dashboard')}>{translate('quotations_dashboard')}</Link></li>
                                  <li>{translate('delivery_method')}</li>
                              </ul>
                              <Link to={'/delivery/method/add/'} className="btn btn-new">{translate('add_delivery_method')}</Link>
                              ...
                              <table className="table list-table">
                                <thead>
                                  <tr>
                                    ...
                                    <th>{translate('delivery_method')}</th>
                                  </tr>
                                </thead>
                                <tbody>
                                {result?
                                    result.delivery.map((delivery , i)=>{
                                        return(
                                               <tr key= {delivery.id}  className = "list_tr" >
                                                  <td>
                                                      <div className="checkbox">
                                                         <input data-id={delivery.id} data-action = {delivery.can_remove}  id={"view-list__cb-"+i} className ="quotation_checkbox" type="checkbox" onClick = {(event)=>this.handleMarkUnmark(event)} />
                                                      <label htmlFor={"view-list__cb-"+i}></label>

                                                      </div>
                                                  </td>  
                                                  <td className = "template-name" onClick={this.handleView.bind(this, delivery.id)} >
                                                    {delivery.name}
                                                  </td>                                         
                                                </tr>
                                              )
...
module.exports = DeliveryMethodList;
