 ...
 onKeyDown(e) {
  ...
  render_header(){
    ...
    var key =''
        ...
        <h1 className="pull-left"><a href="#" title="Saalz"><img src={'/static/front/images/saalz-small.png'} alt="Saalz" height="30" /></a></h1>
                      ...
                      <div className="row top-actions d-lg-flex">
                          <div className="col-xs-12 col-sm-12">
                              <ul className="breadcrumbs-top">
                                ...
                                <Link to={'/customer/invoice/add/'}   className="btn btn-new">
                                       Add Invoice
                                </Link>
                          </div>
                          ...
                          <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                              ...
                              result.Customer.map((list, i)=>{
                                return(
                                      <tr key= {list.id}  className = "list_tr" >
                                        ...
                                        <td data-th="Customer" onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>
                                            <span  className="text-primary">{list.customer}</span>
                                        </td>
                                        <td data-th="Send Email"  style={cursor_pointer}>
                                         {list.checkbox_email == 1?
                                           <span>
                                               <span  data-tip data-for={'envelope_'+list.id} className="glyphicon glyphicon-envelope" style={{'color':list.color}}></span>
                                               <ReactTooltip place="top" id={'envelope_'+list.id} type="info" effect="float">
                                        ...
                                        <td data-th="Invoice Date" onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>{list.invoice_date}</td>
                                        <td data-th="Quotation Number" onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>
                                          <a href="javascript:void(0)" title={"View Details of  "+list.invoice_number}>{list.invoice_number}</a>
                                        </td>
                                        <td data-th="Sales Person" onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>{list.sales_person}</td>
                                        {list.due_dated =='after' ?
                                          <td data-th="Order Date" onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>{list.due_date}</td>
                                        : <td className="isAfterDate" data-th="Order Date" onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>{list.due_date}</td>
                                        }
                                        
                                        <td data-th="Customer" onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>{list.source_document}</td>
                                        <td data-th="Total"  onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>{list.total !='' && list.total !==null ? list.total.toFixed(2): 0} {result && result.currency!==undefined ? result.currency : '' }</td>
                                        <td data-th="Total"  onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>{list.amount_due !='' && list.amount_due !==null ? list.amount_due.toFixed(2): 0} {result && result.currency!==undefined ? result.currency : '' }</td>
                                        <td data-th="Status" onClick={this.handleView.bind(this,list.uuid)} style={cursor_pointer}>{list.status}</td>
                                      </tr>
                                    )
                              })
                             :null
                            ...
                            <tfoot>
                              <tr>
                                ...
                                <td>&nbsp;</td>
                                <td>&nbsp;</td>
                                <td data-th="Total"> {result && result.currency!==undefined ? result.currency : '' }  {result && result.total_amount!==undefined ? result.total_amount.toFixed(2): 0}</td>
                                <td>&nbsp;</td>
                                <td>&nbsp;</td>
                              </tr>
                            </tfoot>
