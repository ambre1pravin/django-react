...
class PaymentTermList extends React.Component {
    ...
    handleView(view_id) {
        browserHistory.push( "/payment/term/view/" + view_id +"/");
    }
    ...
    handleDeleteSelected() {
        ...
        if (can_remomve === false) {
            alert('you can not delete a payment!  ');
            return;
        }
        if (quot_ids.length > 0) {
            $.ajax({
                type: "POST",
                cache: false,
                url: '/payment/term/deletepayment/',
                data: {
                    ajax: true,
                                ...
                                <div className="row top-actions d-lg-flex">
                                    <div className="col-xs-12 col-sm-12">
                                        <ul className="breadcrumbs-top">
                                            ...
                                            <li>{translate('payment_term')} </li>
                                        </ul>
                                        <Link to={'/payment/term/add/'}className="btn btn-new">{translate('add_payment_term')}</Link>
                                    </div>
                                    ...
                                    <span className="dropdown-toggle" data-toggle="dropdown" role="button"
                                          aria-haspopup="true" aria-expanded="true" id="actions">
                                     {translate('label_action')}<i className="fa fa-angle-down push-left-3"></i>
                                        ...
                                        <ul className="list-inline inline-block top-actions-pagination">
                                            ...
                                            <li><a href="javascript:void(0)"
                                                   onClick={this.getNextPrevPage.bind(this, 'prev')}>
                                                <i className="fa fa-chevron-left"></i></a>
                                            </li>
                                            <li><a href="javascript:void(0)"
                                                   onClick={this.getNextPrevPage.bind(this, 'next')}>
                                                    ...
                                                    <th>
                                                        <div className="checkbox">
                                                            <input id="view-list__cb-all" type="checkbox"
                                                                   checked={this.state.checkbox == true ? 'checked' : ''}
                                                                   onClick={(event) => this.handleMarkUnmarkAll(event)}/>
                                                            <label htmlFor="view-list__cb-all"></label>
                                                        </div>
                                                    </th>
                                                    <th>{translate('payment_term')}</th>
                                                </tr>
                                                </thead>
                                                <tbody>
                                                {result ?
                                                    result.payment.map((payment, i) => {
                                                        return (
                                                            <tr key={payment.id} className="list_tr">
                                                                <td>
                                                                    <div className="checkbox">
                                                                        <input data-id={payment.id}
                                                                               data-action={payment.can_remove}
                                                                               id={"view-list__cb-" + i}
                                                                               className="quotation_checkbox"
                                                                               type="checkbox"
                                                                               onClick={(event) => this.handleMarkUnmark(event)}/>
                                                                        <label htmlFor={"view-list__cb-" + i}></label>
                                                                    </div>
                                                                </td>
                                                                <td style={cursor_pointer} className="template-name"
                                                                    onClick={this.handleView.bind(this, payment.uuid)}>
                                                                    <span className="text-primary">{payment.name}</span>
                                                                </td>
                                                                <td style={cursor_pointer} onClick={this.handleView.bind(this, payment.uuid)}>&nbsp;</td>
                                                            </tr>
                                                        )
                                                    })
                                                    : null
...
module.exports = PaymentTermList;
